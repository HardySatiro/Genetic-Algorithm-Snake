...
class ModelEpsilon():
    def __init__(self, n_input, n_hidden_1, n_hidden_2, n_hidden_3, n_classes):
        ...
        ep1 = np.sqrt(2.0 / (n_hidden_1 * n_hidden_2))
        ep2 = np.sqrt(2.0 / (n_hidden_2 * n_hidden_3))
        ep3 = np.sqrt(2.0 / (n_hidden_3 * n_classes))

        self.w1 = tf.Variable(ep1*glorot_init([n_input, n_hidden_1]))
        self.w2 = tf.Variable(ep2*glorot_init([n_hidden_1, n_hidden_2]))
        self.w3 = tf.Variable(ep3*glorot_init([n_hidden_2, n_hidden_3]))
