    ...
    for idx in range(len(ind1)):
        ...
        if idx % 2 == 0:
            for hidden in range(len(ind1[idx])):
                ...
                ind1[idx][hidden][cxpoint:], ind2[idx][hidden][cxpoint:] = ind2[idx][hidden][cxpoint:]*1.03, ind1[idx][
                                                                                                            hidden][
                                                                                                        cxpoint:]*1.03
            ...
            ind1[idx][cxpoint:], ind2[idx][cxpoint:] = ind2[idx][cxpoint:]*1.03, ind1[idx][cxpoint:]*1.03
...
def create_indi(list_indi):
    ...
    for ind in range(len(list_indi)):
        if ind + ind_mut >= len(list_indi):
            a, b = crossover(np.array(list_indi[ind].get_weights()),
                             np.array(list_indi[-(ind + ind_mut) + len(list_indi)].get_weights()))
...
if __name__ == "__main__":
    ...
    try:
        ...
        bests = df[0].values[-(population - qtd_ind_gen):]

        for j in range(qtd_ind_gen):
            list_best_ind.append(create_model())
        for id in bests:
        ...
        while True:  # TRAINING EPOCS
            ...
            # print(len(list_ind_all))
            list_ind_score = []
            list_ind_score_indx = []
            list_best_ind = []
            snake.rond = [random.randrange(snake.block * 2, snake.height_width - snake.block * 2, snake.block),
                          random.randrange(snake.block * 2, snake.height_width - snake.block * 2, snake.block)]
