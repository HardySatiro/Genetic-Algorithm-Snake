...
if __name__ == "__main__":
    ...
    try:
        ...
        #print(epoch, min(list_ind_score), np.mean(list_ind_score), max(list_ind_score))
        file = open(f'resources/csv/{args.version}.csv', 'a+')
        file.write(f'{epoch}, {min(list_ind_score)}, {np.mean(list_ind_score)}, {max(list_ind_score)}\n')  # Give your csv text here.
        file.close()
        ...
        while True:  # TRAINING EPOCS
            ...
            file = open(f'resources/csv/{args.version}.csv', 'a+')
            file.write(
                f'{epoch}, {min(list_ind_score)}, {np.mean(list_ind_score)}, {max(list_ind_score)}\n')  # Give your csv text here.
            file.close()
            # print(df)
            bests = df[0].values[-qtd_ind_gen:]
